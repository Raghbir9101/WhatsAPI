...
const sendMessage = async (req, res) => {
    ...
    if (user.messagesSent >= user.monthlyLimit) {
        res.status(429).json({ error: 'Monthly message limit exceeded' });
        return;
    }
    try {
        ...
        const client = whatsappManager.getClient(user._id.toString(), instanceId);
        if (!client) {
            res.status(400).json({ error: 'WhatsApp client not initialized' });
            return;
        }
        const clientStatus = whatsappManager.getClientStatus(user._id.toString(), instanceId);
        if (clientStatus !== 'ready') {
            res.status(400).json({ error: `WhatsApp client not ready. Status: ${clientStatus}` });
            return;
        }
...
const sendMediaUrl = async (req, res) => {
    ...
    try {
        const instance = await models_1.WhatsAppInstance.findOne({ instanceId, userId: user._id });
        if (!instance) {
            res.status(404).json({ error: 'WhatsApp number instance not found' });
            return;
        }
        ...
        const chatId = (0, helpers_1.formatPhoneNumber)(to);
        const media = await whatsapp_web_js_1.MessageMedia.fromUrl(mediaUrl, { unsafeMime: true });
        const sentMessage = await client.sendMessage(chatId, media, { caption });
...
const getChatInfo = async (req, res) => {
    ...
    if (!instanceId || !phoneNumber) {
        res.status(400).json({ error: 'instanceId and phoneNumber are required' });
        return;
    }
    try {
        const instance = await models_1.WhatsAppInstance.findOne({ instanceId: instanceId, userId: user._id });
        if (!instance) {
            res.status(404).json({ error: 'WhatsApp number instance not found' });
            return;
        }
...
const getMessages = async (req, res) => {
    ...
    try {
        ...
        const totalMessages = await models_1.Message.countDocuments(query);
...
const getConversations = async (req, res) => {
    ...
    try {
        ...
        const conversations = await models_1.Message.aggregate([
            { $match: query },
            {
                $addFields: {
                    contact: {
